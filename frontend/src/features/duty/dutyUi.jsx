...
import { dutyVariant } from './dutyHelpers';

export function DutyDemoNotice() {
  return (
    <p className="rounded-panel border border-default bg-subtle px-4 py-2 text-caption font-medium text-muted">
      Checkpoint-level demo data only. No real authentication, backend, OCR, face recognition, AI,
      notification feed, or government database is connected.
    </p>
  );
}

export function DutyStatusBadge({ status, children }) {
  return (
    <StatusBadge status={status} variant={dutyVariant[status]}>
      {children}
    </StatusBadge>
  );
}

export function DutyRiskBadge({ risk }) {
  return <Badge variant={dutyVariant[risk] || 'neutral'}>{risk}</Badge>;
}
